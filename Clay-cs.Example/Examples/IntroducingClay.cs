...
public class IntroducingClay : IDisposable
{
	...
	private StringCache _string = new StringCache();

	private int _selectedDocumentIndex;


	private Clay_RectangleElementConfig _contentBackgroundConfig = new()
	{
		color = new Clay_Color(90, 90, 90),
	...
	public unsafe void Run()
	{
		...
		[
			new Document
			{
				Title = _string.GetClayString("Squirrels"),
				Contents = _string.GetClayString(
			...
			new Document
			{
				Title = _string.GetClayString("Lorem Ipsum"),
				Contents = _string.GetClayString(
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.")
			},
			new Document
			{
				Title = _string.GetClayString("Vacuum Instructions"),
				Contents = _string.GetClayString(
			...
			new Document { Title = _string.GetClayString("Article 4"), Contents = _string.GetClayString("Article 4") },
			new Document { Title = _string.GetClayString("Article 5"), Contents = _string.GetClayString("Article 5") },
		];
		...
		while (Raylib.WindowShouldClose() == false)
		{
			...
			var scrollDelta = Raylib.GetMouseWheelMoveV();
			Clay.SetPointerState(new Clay_Vector2
			{
				x = mousePos.X,
				y = mousePos.Y,
			}, Raylib.IsMouseButtonDown(0));

			Clay.UpdateScrollContainers(true, new Clay_Vector2
			{
				x = scrollDelta.X,
				y = scrollDelta.Y,
			}, Raylib.GetFrameTime());
			...
			using (new ClayElement(new()
			{
				Id = _string["OuterContainer"],
				Rectangle = new()
				{
					color = new Clay_Color(43, 41, 51)
				},
				Layout = new()
				{
			...
			}))
			{
				using (new ClayElement(new()
				{
					Id = _string["HeaderBar"],
					Rectangle = _contentBackgroundConfig,
					Layout = new()
					{
						sizing = new Clay_Sizing(Clay_SizingAxis.Grow(), Clay_SizingAxis.Fixed(60)),
						padding = Clay_Padding.Hor(16),
						childGap = 16,
						childAlignment = new Clay_ChildAlignment(Clay_LayoutAlignmentX.CLAY_ALIGN_X_CENTER, default)
					}
				}))
				{
					var fileButtonStr = _string["FileButton"];
					var fileMenuStr = _string["FileMenu"];
					...
					using (new ClayElement(new()
					{
						...
						Rectangle = new()
						{
							color = new Clay_Color(140, 140, 140),
						...
						if (isMenuVisible)
						{
							...
							}))
							{
								using (new ClayElement(new()
								{
									...
									Rectangle = new()
									{
										color = new Clay_Color(40, 40, 40),
										cornerRadius = Clay_CornerRadius.All(8),
									}
								}))
								{
									RenderDropdownMenuItem(_string["New"]);
									RenderDropdownMenuItem(_string["Open"]);
									RenderDropdownMenuItem(_string["Close"]);
								}
							}
						}
					}

					RenderHeaderButton(_string["Edit"]);
					RenderHeaderButton(_string["Upload"]);
					RenderHeaderButton(_string["Media"]);
					RenderHeaderButton(_string["Support"]);
					RenderHeaderButton(_string["Upload"]);
				}

				using (new ClayElement(new()
				{
					Id = _string["LowerContent"],
					Layout = new()
					{
						sizing = new Clay_Sizing(Clay_SizingAxis.Grow(), Clay_SizingAxis.Grow()),
						childGap = 16,
				...
				}))
				{
					...
					}))
					{
						...
						for (int i = 0; i < _documents.Length; i++)
						{
							...
							bool isSelected = i == _selectedDocumentIndex;


							if (i == _selectedDocumentIndex)
							{
								using (new ClayElement(new()
								{
									...
									Rectangle = new()
									{
										color = new Clay_Color(120, 120, 120, 255),
										cornerRadius = Clay_CornerRadius.All(8),
									}
								}))
								{
							...
							else
							{
								...
								{
									Layout = sidebarButtonLayout,
									Rectangle = Clay.IsHovered() == false
										? default
										: new()
										{
											color = new Clay_Color(120, 120, 120, 255),
					...
					using (new ClayElement(new()
					{
						Id = _string["MainContent"],
						Scroll = new()
						{
							vertical = true.AsByte()
						},
						Layout = new()
						{
							layoutDirection = Clay_LayoutDirection.CLAY_TOP_TO_BOTTOM,
